...
public class TVertice<T> implements IVertice {
    ...
    @Override
    public void ordenTopologico(LinkedList<IVertice> resultado) {
    setVisitado(true);
    for (IAdyacencia ady : getAdyacentes()) {
        IVertice destino = ady.getDestino();
        if (!destino.getVisitado()) {
            destino.ordenTopologico(resultado);
        }
    }
    resultado.addFirst(this); // Añadir al principio para respetar el orden topológico
}
